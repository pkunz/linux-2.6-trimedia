...
PATCHLEVEL = 6
SUBLEVEL = 14
EXTRAVERSION = .7-tiny1
NAME=Attenuated Albatross
...
#		   -ffreestanding
# remove -Wall to disable -Wunused
CFLAGS 		:= -w0 -D__inline=inline -D__restrict=restrict
AFLAGS		:= -D__ASSEMBLY__
# CILFLAGS	:= --keepunused --doGeneral --doRmattr --doTpcvt --doShadow
CILFLAGS	:= --keepunused --doGeneral --doTpcvt --doShadow
...
vmlinux-main := $(core-y) $(drivers-y) $(net-y) { $(libs-y) }
vmlinux-all  := $(vmlinux-init) $(vmlinux-main)
#
# FIXME: No vmlinux.lds for TriMedia
...
# Also handle biarch i386+x86_64 cross-compiler
ifeq ($(ARCH),x86_64)
ifeq ($(CONFIG_CROSS_COMPILER_DEFAULT_BITS_32),y)
export CPPFLAGS_vmlinux.lds += -Ui386
endif
endif
...
clean: archclean $(clean-dirs)
	...
	@find . $(RCS_FIND_IGNORE) \
	 	\( -name '*.[oaE]' -o -name '*.ko' -o -name '.*.cmd' \
		-o -name '.*.d' -o -name '.*.tmp' -o -name '*.mod.c' -o -name '*.cil.c' \
		-o -name '*.symtypes' \) \
...
ifeq ($(CONFIG_CROSS_COMPILER_DEFAULT_BITS_32),y)
AFLAGS_.tmp_kallsyms1.o = -Wa,-64
AFLAGS_.tmp_kallsyms2.o = -Wa,-64
endif
