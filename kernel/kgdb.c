		...
		case 'q':
			...
			case 'c':
				if (memcmp(remcom_in_buffer + 1,
					 ...
					 */
					strcpy(remcom_out_buffer,system_utsname.machine);
...
#endif
					strcat(remcom_out_buffer,system_utsname.release);
					strcat(remcom_out_buffer,",");
...
void breakpoint(void)
{
	...
	/* Force set the attached state of kgdb because the exception loop
	 * should be required to activate on all archs, as this is the
	 * internal kgdb breakpoint routine.
	 */
	if (kgdb_io_ops.read_char)
	{
		kgdb_connected = 1;
