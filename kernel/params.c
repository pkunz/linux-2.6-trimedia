		     ...
		     int (*handle_unknown)(char *param, char *val))
{
	...
	for (i = 0; i < num_params; i++) {
		ASSERT(params!=NULL);
		if (parameq(param, params->name)) {
			DEBUGP("They are equal!  Calling %p\n",
			       params->set);
			return params->set(val, params);
		}
		params = params->next;
		if(params==NULL){
			DEBUGP("to end of __kernel_param_list, goto handle_unknown\n");
