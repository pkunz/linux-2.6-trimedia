 ...
 *  $Id: 8250.c 1.3 2005/10/18 09:43:20EDT Alexandre Bounine (alexandre bounine) Exp  $
 *
...
#ifdef CONFIG_TSI108_BRIDGE /* for TSI108_REV_A1 */
		iir = (*(unsigned int*)(up->port.membase + 0x00) >> 8) & 0xff; /* read as 32-bit word */
#else
		iir = serial_in(up, UART_IIR);
#endif
...
static int serial8250_startup(struct uart_port *port)
{
	...
	(void) serial_inp(up, UART_LSR);
	(void) serial_inp(up, UART_RX);
#ifndef CONFIG_TSI108_BRIDGE /* for TSI108_REV_A1 */
	(void) serial_inp(up, UART_IIR);
#endif
	(void) serial_inp(up, UART_MSR);
...
#if !defined(CONFIG_SBC8560) && !defined(CONFIG_TAIGA)
	/*
...
#ifdef CONFIG_SERIAL_8250_RSA
module_param_end(share_irqs, uint, 0644);
module_param_array_start(probe_rsa, ulong, &probe_rsa_count, 0444,share_irqs);
MODULE_PARM_DESC(probe_rsa, "Probe I/O ports for RSA");
#else
module_param_one(share_irqs, uint, 0644);
#endif
