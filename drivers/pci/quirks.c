...
DECLARE_PCI_FIXUP_HEADER_ONE(PCI_VENDOR_ID_NETMOS, PCI_ANY_ID, quirk_netmos);
#endif 
...
{
	struct pci_fixup *start;

	switch(pass) {
	case pci_fixup_early:
		start = __pci_fixup_early_list_ptr;
		break;

	case pci_fixup_header:
		start = __pci_fixup_header_list_ptr;
		break;

	case pci_fixup_final:
		start = __pci_fixup_final_list_ptr;
		break;

	case pci_fixup_enable:
		start = __pci_fixup_enable_list_ptr;
		break;
