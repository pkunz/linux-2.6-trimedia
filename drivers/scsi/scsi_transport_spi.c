...
spi_dv_device(struct scsi_device *sdev)
{
	...
	/*
	 * If scsi_transport_spi driver is statically linked
	 * kernel crashes during bootup since dev->driver 
	 * pointer is NULL. Hence changing the "dev_printk"
	 * to prink.
	 */
	printk(KERN_INFO "Beginning Domain Validation\n");

	spi_dv_device_internal(sdev, buffer);

	printk(KERN_INFO "Ending Domain Validation\n");
...
void spi_display_xfer_agreement(struct scsi_target *starget)
{
	...
	if (tp->offset > 0 && tp->period > 0) {
		...
		printk(KERN_INFO "%sasynchronous.\n",
				tp->width ? "wide " : "");
	}
}
