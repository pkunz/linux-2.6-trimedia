...
/* XXX: compile hack for v2.6.14. SYY */
...
{
	struct async_struct *info = (struct async_struct *)tty->driver_data;
	/* unsigned long flags; not used. SYY */
 ...
 */
/* static int rs_initiated;  not used. SYY */
...
#if 0
	// Start console, if console check on us earlier
	if (rs_initiated < 0) {
		brcm_serial_console_init();
	}
	rs_initiated = 1;
#endif
	return 0;
