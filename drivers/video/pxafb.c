...
static struct constraints pxafb_constraints = {
	...
	.param = {{DPM_MD_PLL_LCD, 52000, 52000},},
};
#endif
...
#define WAIT_FOR_LCD_INTR_IDLE(reg,intr,timeout) ({	\
        int __done =0;                          \
        int __t = timeout*100;			\
        while (__t) {                           \
                __done = (reg) & (intr);        \
...
static int pxafb_activate_var(struct fb_var_screeninfo *var, struct pxafb_info *fbi)
{
	...
	pr_debug("palette_mem_size = 0x%08lx\n", (u_long) palette_mem_size);
	...
	void *newop)
{
	if(pxafbi != NULL){
		...
		LCCR0 &= ~LCCR0_LDM;	/* Enable LCD Disable Done Interrupt */
		LCCR0 |= LCCR0_DIS;	/* Disable LCD Controller */
		WAIT_FOR_LCD_INTR_IDLE(LCSR0,LCSR_LDD,20);
#ifdef CONFIG_PXA27x
...
static int overlay2fb_map_RGB_memory(struct overlayfb_info *fbi)
{
	...
	pr_debug("FB2, RGB: map %d bytes (%dx%d), nbytes %d\n",
		fbi->fb.fix.smem_len,
...
static int overlay2fb_enable(struct overlayfb_info *fbi)
{
	...
	pr_debug("FB2, enable: %dx%d, bpp %x, format %d, xpos %d, ypos %d\n",
...
static int overlay2fb_disable_RGB(struct fb_info *info)
{
	...
	if (!WAIT_FOR_LCD_INTR(LCSR1, LCSR1_BS2, 100)) {
		pr_debug(KERN_WARNING "%s: timeout\n", __FUNCTION__);
		return -1;
	}
	return 0;
}
...
static int overlay2fb_enable_YUV420(struct overlayfb_info *fbi)
{
	...
	if (fbi->format) {
		...
		pr_debug(KERN_INFO "%s: fbi->format == 0 ???\n", __FUNCTION__);
	}
...
int __init pxafb_probe(struct device *dev)
{
	...
	ret = request_irq(IRQ_LCD, pxafb_handle_irq, SA_INTERRUPT, "LCD", pxafbi);
	if (ret) {
...
//      Doesn't work for 6.14 yet.
	...
	if (!cursorfb) {
		ret = -ENOMEM;
		pr_debug(KERN_ERR "PXA FB: cursorfb_init_fbinfo failed\n");
		goto failed;
	}
