...
#if defined(CONFIG_GFAR_NAPI) && defined(CONFIG_NET_POLL_CONTROLLER)
/* The NAPI driver needs to be ported to work with the NET_POLL_API
 * using it it in the current state will hang the ethernet 
 */
#error CONFIG_GFAR_NAPI and CONFIG_NET_POLL_CONTROLLER are mutually exclusive at this time, please use one or the other.
#endif
...
static void gfar_poll_controller(struct net_device *dev);
...
static int gfar_probe(struct device *device)
{
	...
	dev->poll_controller = gfar_poll_controller;
#endif
...
int startup_gfar(struct net_device *dev)
{
	...
	/* set the PAL bits */
	rctrl |= (priv->padding << 16) & RCTRL_PAL_MASK;
...
static inline void gfar_rx_checksum(struct sk_buff *skb, struct rxfcb *fcb)
{
	...
	/*
	 * Fix the checksum error determine logical.
	 * if (fcb->cip && !fcb->eip && fcb->ctu && !fcb->etu)
	 */
	if (!fcb->eip && !fcb->etu && (fcb->cip || fcb->ctu))
...
int gfar_clean_rx_ring(struct net_device *dev, int rx_work_limit)
{
	...
	/*
         * look for the first frame that is not empty (FRAMES COULD BE UNSYNCHRONIZED)
         * This routine is entered via interrupt and something should be here.
         */
	...
	priv->cur_rx = (struct rxbd8 *)bdp;


#ifndef final_version
	if (bdp_save != bdp)
	printk(KERN_ALERT "%s: UnSYNC'd PACKET new buffer ptr: %x orig buffer ptr: %x\n", __FUNCTION__, (int)bdp, (int)bdp_save);
#endif
