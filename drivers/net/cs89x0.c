...
static int net_start_tx(struct sk_buff *skb, struct net_device *dev);
static int net_send_packet(struct net_device *dev);  
...
cs89x0_probe1(struct net_device *dev, int ioaddr, int modular)
{
	...
	dev->hard_start_xmit 	= net_start_tx;
...
static int net_start_tx(struct sk_buff *skb_orig, struct net_device *dev)
{
	struct net_local *lp = netdev_priv(dev);
	unsigned entry;
	unsigned long flags;
	...
	spin_lock_irqsave(&lp->lock, flags);
	// XXX netif_stop_queue(dev);

	if ((int)(lp->cur_tx - lp->dirty_tx) >= TX_QUEUE_LIMIT) {
...
static int net_send_packet(struct net_device *dev)
{
	...
	while (!lp->txbusy && (lp->dirty_tx != lp->cur_tx)) {
		do {
			/* initiate a transmit sequence */
			writeword(dev, TX_CMD_PORT, lp->send_cmd);
			writeword(dev, TX_LEN_PORT, lp->tx_skbuff[entry]->len);
	
			/* Test to see if the chip has allocated memory for the packet */
			status = readreg(dev, PP_BusST);
			if (status & TX_BID_ERROR) {
				dev_kfree_skb(lp->tx_skbuff[entry]);
				lp->tx_skbuff[entry] = NULL;
				entry = (++lp->dirty_tx) % TX_RING_SIZE;

				txLoop = 0;
			} else {
				if (status & READY_FOR_TX_NOW) {
					/* Write the contents of the packet */
					outsw(dev->base_addr + TX_FRAME_PORT,
					  lp->tx_skbuff[entry]->data,
					  (lp->tx_skbuff[entry]->len+1)>>1);
	...
	while ((status = readword(dev, ISQ_PORT))) {
		...
		case ISQ_TRANSMITTER_EVENT:
			...
			if (netif_queue_stopped(dev)) {
				...
				if (netpoll_trap()) {
					printk(KERN_INFO "KGDB: TRAP unstucked queue\n");
		...
		case ISQ_BUFFER_EVENT:
			...
			if (status & READY_FOR_TX || status & TX_UNDERRUN) {
				net_send_packet(dev);
				if (netif_queue_stopped(dev)) {
					netif_wake_queue(dev);
#ifdef CONFIG_NETPOLL_TRAP
