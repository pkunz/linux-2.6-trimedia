...
#define SBMAC_TRUESIZE_REQ (sizeof (struct sk_buff) +	\
			    16 + ENET_PACKET_SIZE +	\
			    SMP_CACHE_BYTES * 2 +	\
			    ETHER_ALIGN)
...
static int sbdma_add_rcvbuffer(sbmacdma_t *d,struct sk_buff *sb)
{
	...
	if (sb_new == NULL) {
		...
		if (sb_new == NULL) {
			 ...
			 */
			skb_shinfo(sb_new)->tso_size = 0;
			skb_shinfo(sb_new)->tso_segs = 0;
		}
#endif
	...
	for (;; d->sbdma_remptr = SBDMA_NEXTBUF(d,sbdma_remptr)) {
		...
		if (poll) {
			...
			if (atomic_read(&sb->users) == 1 &&
			    ...
			    sb->truesize >= SBMAC_TRUESIZE_REQ &&
			    sb->data_len == 0 &&
 ...
 *  	   nothing
 ********************************************************************* */
static void sbmac_poll_controller(struct net_device *dev)
{
	struct sbmac_softc *sc = netdev_priv(dev);
	disable_irq(dev->irq);
...
#ifdef CONFIG_NET_POLL_CONTROLLER
	dev->poll_controller	= sbmac_poll_controller;
#endif
#ifdef CONFIG_SBMAC_NAPI
	dev->poll               = sbmac_poll;
	dev->weight             = 16;
