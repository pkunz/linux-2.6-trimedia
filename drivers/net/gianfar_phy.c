...
static int bcm5421s_config_aneg(struct gfar_mii_info *mii_info)
{
       struct gfar_private *priv = netdev_priv(mii_info->dev);

       /* Write the appropriate value to the PHY reg */
       if (priv->einfo->device_flags & FSL_GIANFAR_DEV_HAS_GIGABIT)
               write_phy_reg(mii_info->dev, mii_info->mii_id, MII_BCM5421S_CONTROL, MII_BCM5421S_CONTROL_INIT);
       else
               write_phy_reg(mii_info->dev, mii_info->mii_id, MII_BCM5421S_CONTROL, MII_BCM5421S_CR_INIT);
...
static int bcm5421s_read_status(struct gfar_mii_info *mii_info)
{
       u16 status;
       int err;
        ...
        * are as set */
       if (mii_info->autoneg && mii_info->link) {
               status = phy_read(mii_info, 0x19);

               /* Get the speed and duplexity */
               switch((status & 0x700) >> 8) {
                       case 1:
                               mii_info->speed = SPEED_10;
                               mii_info->duplex = DUPLEX_HALF;
                               break;
...
static struct phy_info phy_info_bcm5421s= {
       ...
       .read_status    = bcm5421s_read_status,
};
...
#ifdef CONFIG_MPC8540_EVAL
/*For Intel LXT971*/
static struct phy_info phy_info_LXT971A={
        .phy_id         = 0x00137800,
        .phy_id_mask    = 0xffffff00,
        .name           = "Intel LXT971A",
        .features       = MII_BASIC_FEATURES,
        .config_aneg    = genmii_config_aneg,/*LXT971A_config_aneg,*/
	.read_status    = genmii_read_status,/*&LXT971A_read_status,
	 *.ack_interrupt  = &LXT971A_ack_interrupt,
	 *.config_intr    = &LXT971A_config_intr,*/
};
#endif
