...
#ifdef CONFIG_MV643XX_BCM5421S
#include <asm/mv64x60_defs.h>
#include "mv64360_phy.h"
#endif
...
#ifdef CONFIG_POWERPMC280
#define ETH_PHY0_IRQ_NUM        76
#define MV64360_NUM_ETHPORTS      3

static int revB = 1;
static struct net_device *stored_dev[MV64360_NUM_ETHPORTS] = { NULL, NULL, NULL };
static u_char mac_addrs[18];
extern u_char ppmc280_spd[];

extern uint i2c_word_read (u_char, uint, unsigned int, u_char *);
extern uint i2c_byte_read (u_char, u_char, uint, u_char *);
extern int get_eeprom_size (u_char);
#endif
...
{
#ifdef CONFIG_MV643XX_BCM5421S
	struct mv643xx_private *mp = dev->priv;
	unsigned int port_num = mp->port_num;
	unsigned int port_status, phy_reg_data;
#ifndef CONFIG_POWERPMC280
	unsigned int phy_id_low, phy_id_high, phy_id;

	eth_port_read_smi_reg(port_num, DSC2_ENET_PHY_PHY_ID_MSB, &phy_id_high);
	eth_port_read_smi_reg(port_num, DSC2_ENET_PHY_PHY_ID_LSB, &phy_id_low);
	phy_id = ((phy_id_high << 16) | phy_id_low) & PHY_ID_REVISION_MASK;
//    printk("%s: PHY id 0x%x\n", dev->name, phy_id);
	...
	if (!(phy_reg_data & 0x20)) {
		...
		printk(KERN_NOTICE "%s: Ethernet status : Link up", dev->name);
		printk(", %s",
		       (port_status & BIT2) ? "Full Duplex" : "Half Duplex");
		if (port_status & BIT4)
			printk(", Speed 1 Gbps");
		else
			printk(", %s", (port_status & BIT5) ? "Speed 100 Mbps" :
			       "Speed 10 Mbps");
		printk("\n");
	}
#endif
...
#ifdef CONFIG_MV643XX_BCM5421S
#ifndef CONFIG_POWERPMC280
#define ETH_PHY_IRQ_NUM 71

static void ethernet_phy_interrupt_disable(int eth_port_num);
static void ethernet_phy_interrupt_enable(int eth_port_num);

static int real_opens;
static int phy_id;
#endif /* ! CONFIG_POWERPMC280 */

irqreturn_t mv643xx_eth_phy_int_handler(int irq, void *dev_id,
					struct pt_regs * regs);
#endif /* CONFIG_MV643XX_BCM5421S */
...
#ifdef CONFIG_MV643XX_BCM5421S
#ifdef CONFIG_POWERPMC280
	/* Enable PHY interrupt to monitor PHY status */
	if (!revB) {
		err = request_irq(ETH_PHY0_IRQ_NUM + port_num,
		  mv643xx_eth_phy_int_handler,
		  (SA_INTERRUPT | SA_SAMPLE_RANDOM), dev->name, dev);
		if (err) {
			printk("mv643xx_eth_open: ERROR: Cannot assign IRQ number to MV64360_eth%d\n", port_num);
			err = -EAGAIN;
			goto out;
		} else
			printk("mv643xx_eth_open: Assigned IRQ %d to MV64360_eth%d\n", ETH_PHY0_IRQ_NUM + port_num, port_num);
	} else {
		/*
		 * check if other port is open
		 * the structure stored_dev is unprotected & requries protection
		 */
		dev_hold(dev);
		stored_dev[port_num] = dev;
		if ((port_num == 0 && stored_dev[1] == NULL) ||
		  (port_num == 1 && stored_dev[0] == NULL)) {
			err = request_irq(ETH_PHY0_IRQ_NUM,
			  mv643xx_eth_phy_int_handler,
			  (SA_INTERRUPT | SA_SAMPLE_RANDOM), "eth_phy",
			  stored_dev[port_num]);
			if (err) {
				printk("mv643xx_eth_open: ERROR: Cannot assign PHY IRQ number to MV64360_eth%d\n", port_num);
				stored_dev[port_num] = NULL;
				err = -EAGAIN;
				goto out;
			} else {
				got_port_phy_irq = 1;
				printk("mv64360_eth_open: Assigned IRQ %d for PHY to MV64360_eth%d\n", ETH_PHY0_IRQ_NUM, port_num);
			}
		}
	}
#else
	if (!real_opens) {
		err = request_irq(ETH_PHY_IRQ_NUM, mv643xx_eth_phy_int_handler,
			SA_SHIRQ | SA_SAMPLE_RANDOM, "BCM5421S PHY", &phy_id);
		if (err) {
			printk(KERN_ERR "Cannot obtain irq for MV643XX BCM5421S phy interrupt\n");
			err = -EAGAIN;
			goto out_free;
		}
	}
...
#ifdef CONFIG_POWERPMC280
		if (!revB)
			free_irq(ETH_PHY0_IRQ_NUM + port_num, dev);
		else {
			if (got_port_phy_irq)
				free_irq(ETH_PHY0_IRQ_NUM, stored_dev[port_num]);
			stored_dev[port_num] = NULL;
			dev_put(dev);
		}
#endif
...
#ifdef CONFIG_MV643XX_BCM5421S
#ifndef CONFIG_POWERPMC280
	ethernet_phy_interrupt_enable(port_num);
	real_opens++;
#endif
#endif
...
static int mv643xx_eth_real_open(struct net_device *dev)
{
	...
	reg_data &= ~ETH_FORCE_LINK_PASS;
	reg_data &= ~ETH_DO_NOT_FORCE_LINK_FAIL;
	/* Write Port Serial Control Register */
	mv_write(MV643XX_ETH_PORT_SERIAL_CONTROL_REG(port_num), reg_data);
#endif /* CONFIG_POWERPMC280 */

#if 0
	/* Enable, then restart auto-negotiation */
	eth_port_read_smi_reg(port_num, 0, &phy_reg_data);
	phy_reg_data |= BIT12;	/* Enable auto-negotiation */
	eth_port_write_smi_reg(port_num, 0, phy_reg_data);
	mdelay(10);
	phy_reg_data |= BIT9;	/* Restart auto-negotiation */
	eth_port_write_smi_reg(port_num, 0, phy_reg_data);
	mdelay(10);
#endif

#ifdef CONFIG_POWERPMC280
	/* Clear SMI interrupt registers by reading */
	eth_port_read_smi_reg(port_num, BCM5421S_PHY_INT_STAT, &phy_reg_data);

	/* Enable SMI port status change interrupt. */
	eth_port_read_smi_reg(port_num, BCM5421S_PHY_INT_MASK, &phy_reg_data);
	eth_port_write_smi_reg(port_num, BCM5421S_PHY_INT_MASK, phy_reg_data & ~(BIT1));
#else

	/* Setup the PHY for autonegotiation by advertising capabilities */
	eth_port_read_smi_reg(port_num, DSC2_ENET_PHY_AUTONEG_ADV,
			      &phy_reg_data);
	phy_reg_data &= ~DSC2_ENET_PHY_ANA_100T4_CAPS;	/* no T4 capability */
	phy_reg_data |= DSC2_ENET_PHY_ANA_10T_HD_CAPS;
	phy_reg_data |= DSC2_ENET_PHY_ANA_10T_FD_CAPS;
	phy_reg_data |= DSC2_ENET_PHY_ANA_100TX_HD_CAPS;
	phy_reg_data |= DSC2_ENET_PHY_ANA_100TX_FD_CAPS;

	phy_reg_data &= ~(DSC2_ENET_PHY_ANA_ASM_DIR | DSC2_ENET_PHY_ANA_PAUSE);

	eth_port_write_smi_reg(port_num, DSC2_ENET_PHY_AUTONEG_ADV,
			       phy_reg_data);

	/* 1000Base-T control register */
	eth_port_read_smi_reg(port_num, DSC2_ENET_PHY_1000T_CONTROL,
			      &phy_reg_data);
	phy_reg_data &= ~DSC2_ENET_PHY_CR1000T_HD_CAPS;	/* no gigabit half-duplex */

	phy_reg_data |= DSC2_ENET_PHY_CR1000T_FD_CAPS;
	eth_port_write_smi_reg(port_num, DSC2_ENET_PHY_1000T_CONTROL,
			       phy_reg_data);

	/* MII Control Register */

	phy_reg_data =
	    (DSC2_ENET_PHY_CR_AUTO_NEG_EN | DSC2_ENET_PHY_CR_RESTART_AUTO_NEG);

	eth_port_write_smi_reg(port_num, DSC2_ENET_PHY_MII_CONTROL,
			       phy_reg_data);
#endif /* CONFIG_POWERPMC280 */
#endif /* CONFIG_MV643XX_BCM5421S */
	 ...
	 */
	mv_write(MV643XX_ETH_MAXIMUM_TRANSMIT_UNIT(port_num), 0x0);
	if (!eth_port_link_is_up(port_num)) {
		printk("mv643xx_eth_real_open: link is down\n");
		netif_carrier_off(dev);
		netif_stop_queue(dev);
	}
	else {
...
static int mv643xx_eth_real_stop(struct net_device *dev)
{
	...
	eth_port_write_smi_reg(port_num, 0x1B, 0xffff);
#endif
...
#ifdef CONFIG_MV643XX_BCM5421S
#ifndef CONFIG_POWERPMC280
	ethernet_phy_interrupt_disable(mp->port_num);
	real_opens--;
	if (!real_opens) {
		free_irq(ETH_PHY_IRQ_NUM, &phy_id);
	}
#endif
#endif
...
#ifdef CONFIG_POWERPMC280
	if (!revB)
		free_irq(ETH_PHY0_IRQ_NUM + mp->port_num, dev);
	else {
		stored_dev[mp->port_num] = NULL;
		if (!stored_dev[0] && !stored_dev[1])
			free_irq(ETH_PHY0_IRQ_NUM, stored_dev);
		dev_put(dev);
	}
#endif
...
static int __init mv643xx_init_module(void)
{
	...
	if (eeprom_type == 2) {
		i2c_byte_read(0xA8, 0, 18, mac_addrs);
		revB = 0;
	} else {
		i2c_word_read(0xA8, 0x1ff4, 12, mac_addrs);
		if (((ppmc280_spd[0] & 0x7) == 0) || ((ppmc280_spd[0] & 0x7) == 0x7))
			revB = 0;
	}
#endif /* CONFIG_POWERPMC280 */
...
typedef struct _eth_win_param {
       ...
       unsigned short access_ctrl;     /* Access ctrl register. see above macro
s */
} ETH_WIN_PARAM;

#define MV64x60_EWIN_ACCESS_FULL (BIT1 | BIT0)
#define MV64360_INTERNAL_SRAM_SIZE 0x00040000
#define MV64x60_EBAR_TARGET_CBS 0x00000002
 ...
 *                                      
 *******************************************************************************/
static void eth_set_access_control(unsigned int eth_port_num, ETH_WIN_PARAM * param)
{
	unsigned int access_prot_reg;

	/* Set access control register */
	access_prot_reg = mv_read(MV643XX_ETH_ACCESS_PROTECTION_REG
				      (eth_port_num));
	access_prot_reg &= (~(3 << (param->win * 2)));	/* clear window permission */
	access_prot_reg |= (param->access_ctrl << (param->win * 2));
	mv_write(MV643XX_ETH_ACCESS_PROTECTION_REG(eth_port_num),
		     access_prot_reg);

	/* Set window Size reg (SR) */
	mv_write((MV643XX_ETH_SIZE_REG_0 + (MV64x60_ETH_SIZE_REG_GAP * param->win)),
		     (((param->size / 0x10000) - 1) << 16));

	/* Set window Base address reg (BA) */
	mv_write((MV643XX_ETH_BAR_0 + (MV64x60_ETH_BAR_GAP * param->win)),
		     (param->target | param->attributes | param->base_addr));
	/* High address remap reg (HARR) */
	if (param->win < 4)
		mv_write((MV643XX_ETH_HIGH_ADDR_REMAP_REG_0 +
			      (MV64x60_ETH_HIGH_ADDR_REMAP_REG_GAP * param->win)),
			     param->high_addr);

	/* Base address enable reg (BARER) */
	if (param->enable == 1)
		mv_write(MV64360_ENET2MEM_BAR_ENABLE,
		  mv_read(MV64360_ENET2MEM_BAR_ENABLE) & ~(1 << param->win));
	else
		mv_write(MV64360_ENET2MEM_BAR_ENABLE,
		  mv_read(MV64360_ENET2MEM_BAR_ENABLE) | (1 << param->win));
}
...
{
	u32 result = 0;
	u32 enable = mv_read(MV64360_CPU_BAR_ENABLE);
	if (enable & (1 << bank))
		return 0;
	if (bank == BANK0)
		result = mv_read(MV64x60_CPU2MEM_0_BASE);
	if (bank == BANK1)
		result = mv_read(MV64x60_CPU2MEM_1_BASE);
	if (bank == BANK2)
		result = mv_read(MV64x60_CPU2MEM_2_BASE);
	if (bank == BANK3)
		result = mv_read(MV64x60_CPU2MEM_3_BASE);
	result &= 0x0000ffff;
	result = result << 16;
	return result;
}
...
u32 mv_get_dram_bank_size(MEMORY_BANK bank)
{
	...
	if (enable & (1 << bank))
		return 0;
	if (bank == BANK0)
		result = mv_read(MV64x60_CPU2MEM_0_SIZE);
	if (bank == BANK1)
		result = mv_read(MV64x60_CPU2MEM_1_SIZE);
	if (bank == BANK2)
		result = mv_read(MV64x60_CPU2MEM_2_SIZE);
	if (bank == BANK3)
		result = mv_read(MV64x60_CPU2MEM_3_SIZE);
	result += 1;
	result &= 0x0000ffff;
	result = result << 16;
	return result;
...
{
	u32 result;
	result = mv_read(MV64360_CPU2SRAM_BASE);
	result &= 0x0000ffff;
	result = result << 16;
	return result;
}
...
static void eth_port_init(struct mv643xx_private *mp)
{
	...
	win_param.attributes = MV64x60_EBAR_ATTR_DRAM_CS0;	/* Enable DRAM bank   */
#ifndef CONFIG_NOT_COHERENT_CACHE
	win_param.attributes |= MV64360_IDMA2MEM_SNOOP_WB;
#endif
	...
	win_param.access_ctrl = MV64x60_EWIN_ACCESS_FULL;	/* Enable full access */

	/* Set the access control for address window (EPAPR) READ & WRITE */
	eth_set_access_control(mp->port_num, &win_param);

	/* Set access parameters for DRAM bank 1 */
	win_param.win = ETH_WIN1;	/* Use Ethernet window 1 */
	win_param.target = ETH_TARGET_DRAM;	/* Window target - DDR */
	win_param.attributes = MV64x60_EBAR_ATTR_DRAM_CS1;	/* Enable DRAM bank */
#ifndef CONFIG_NOT_COHERENT_CACHE
	win_param.attributes |= MV64360_IDMA2MEM_SNOOP_WB;
#endif
	win_param.high_addr = 0;
	...
	win_param.target = ETH_TARGET_DRAM;	/* Window target - DDR */
	win_param.attributes = MV64x60_EBAR_ATTR_DRAM_CS2;	/* Enable DRAM bank */
#ifndef CONFIG_NOT_COHERENT_CACHE
	win_param.attributes |= MV64360_IDMA2MEM_SNOOP_WB;
#endif
	win_param.high_addr = 0;
	...
	win_param.target = ETH_TARGET_DRAM;	/* Window target - DDR */
	win_param.attributes = MV64x60_EBAR_ATTR_DRAM_CS3;	/* Enable DRAM bank */
#ifndef CONFIG_NOT_COHERENT_CACHE
	win_param.attributes |= MV64360_IDMA2MEM_SNOOP_WB;
#endif
	win_param.high_addr = 0;
	...
	win_param.target = MV64x60_EBAR_TARGET_CBS;	/* Target - Internal SRAM */
	win_param.attributes = MV64x60_EBAR_ATTR_CBS_SRAM | MV64x60_EBAR_ATTR_CBS_SRAM_BLOCK0;
	win_param.high_addr = 0;
	win_param.base_addr = mv_get_internal_sram_base();	/* Get base addr */
	win_param.size = MV64360_INTERNAL_SRAM_SIZE;	/* Get bank size - 256K */
	win_param.enable = 1;	/* Enable the access */
	win_param.access_ctrl = MV64x60_EWIN_ACCESS_FULL;	/* Enable full access */
...
#ifdef CONFIG_MV643XX_BCM5421S
	mv_write(MV643XX_ETH_TX_QUEUE_0_TOKEN_BUCKET_COUNT(port_num), 0x3fffffff);
	mv_write(MV643XX_ETH_TX_QUEUE_0_TOKEN_BUCKET_CONFIG(port_num), 0x03fffcff);
...
#ifdef CONFIG_POWERPMC280
	mv_write(MV643XX_ETH_PHY_ADDR_REG, 0x00002861);
#else
...
#ifdef CONFIG_MV643XX_BCM5421S
	while (!eth_port_link_is_up(eth_port_num)) {
		time_out--;
		if (time_out == 0) {
			printk("mv643xx_eth: ethernet_phy_reset - no link\n");
			return;
		}
	}
#endif
...
#ifdef CONFIG_MV643XX_BCM5421S
/* BCM5421S PHY support */
#ifndef CONFIG_POWERPMC280
static void ethernet_phy_setup_link(int eth_port_num);

/*******************************************************************************
 * ethernet_phy_interrupt_enable - Enable PHY interrupt.
 *
 * DESCRIPTION:
 *       This routine enables the given ethernet port PHY interrupt.
 *
 * INPUT:
 *		int eth_port_num   Ethernet Port number.
 *
 * OUTPUT:
 *       None.
 *
...
static void ethernet_phy_interrupt_enable(int eth_port_num)
{
	...
	eth_port_read_smi_reg(eth_port_num, BCM5421S_PHY_INT_MASK,
			      &phy_reg_data);
	phy_reg_data &= ~BCM5421S_PHY_INT_LSD_CHANGE;
	eth_port_write_smi_reg(eth_port_num, BCM5421S_PHY_INT_MASK,
			       phy_reg_data);
}

/*******************************************************************************
 * ethernet_phy_interrupt_disable - Disable PHY interrupt.
 *
 * DESCRIPTION:
 *       This routine disables the given ethernet port PHY interrupt.
 *
 * INPUT:
 *		int eth_port_num   Ethernet Port number.
 *
 ...
 * mv643xx_eth_phy_int_handler
 *
 * PHY interrupt handler for the gigbit ethernet ports
 *
 * Input : irq - irq number (not used)
 *         dev_id - a pointer to the required interface's data structure
 *         regs   - not used
					...
					struct pt_regs * regs)
{
	...
	if (!revB) {
		...
		eth_port_read_smi_reg(port_num, BCM5421S_PHY_INT_STAT, &phy_int_cause);
		/* PHY status changed */
		if (phy_int_cause & BIT1) {
			...
			if (!(phy_reg_data & 0x20)) {
				...
				netif_wake_queue(dev);
	      			...
	      			/*
				 * Start all TX queues on ethernet port.
				 * This is good in case of previous packets
				 * where not transmitted, due to link down and
				 * this command re-enables all TX queues.
				 * Note that it is possible to get a TX
				 * resource error interrupt after issuing
				 * this, since not all TX queues are enabled,
				 * or has anything to send.
				 */
				mv_write(MV643XX_ETH_TRANSMIT_QUEUE_COMMAND_REG(port_num), 0xff);
			}
			mv643xx_eth_print_phy_status(dev);
		}
	} else {
		for (port_num=0; port_num<2; port_num++) {
			dev = stored_dev[port_num];
			/* read interrupt cause register */
			eth_port_read_smi_reg(port_num, BCM5421S_PHY_INT_STAT, &phy_int_cause);
			/* PHY status changed */
			if (phy_int_cause & BIT1) {
				...
				if (!(phy_reg_data & 0x20)) {
					...
					dev->flags |= (IFF_RUNNING);
		      			...
		      			/*
					 * Start all TX queues on ethernet port.
					 * This is good in case of previous
					 * packets where not transmitted, due
					 * to link down and this command
					 * re-enables all TX queues.
					 * Note that it is possible to get a TX
					 * resource error interrupt after
					 * issuing this, since not all TX
					 * queues are enabled, or has anything
					 * to send.
					 */
					mv_write(MV643XX_ETH_TRANSMIT_QUEUE_COMMAND_REG(port_num), 0xff);
				}
...
#else	/* ! CONFIG_POWERPMC280 */
irqreturn_t mv643xx_eth_phy_int_handler(int irq, void *dev_id,
					struct pt_regs * regs)
{
	unsigned int port_num;
	unsigned int phy_int_mask;
	unsigned int phy_int_status;

	for (port_num = 0; port_num <= 2; port_num++) {
		if (mv_read(
		  MV643XX_ETH_PORT_SERIAL_CONTROL_REG(port_num)) &
		  ETH_SERIAL_PORT_ENABLE) {
			eth_port_read_smi_reg(port_num, BCM5421S_PHY_INT_MASK,
					      &phy_int_mask);
			phy_int_mask &= BCM5421S_PHY_INT_LSD_CHANGE;
			if (phy_int_mask ^ BCM5421S_PHY_INT_LSD_CHANGE) {
				eth_port_read_smi_reg(port_num,
						      BCM5421S_PHY_INT_STAT,
						      &phy_int_status);
				if (phy_int_status &
				    BCM5421S_PHY_INT_LSD_CHANGE) {
					ethernet_phy_setup_link(port_num);
				}
			}
		}
	}
...
#ifndef CONFIG_POWERPMC280
/*******************************************************************************
 * ethernet_phy_setup_link 
 *
 * DESCRIPTION:
 *       This routine sets up the link parameters by reading status 
 *	 from the Broadcom BCM5421S PHY
 *
 * INPUT:
 *		int eth_port_num   Ethernet Port number.
 *
...
#define ETH_TQCR_ENQ7		(1<<7)	/* Enable Tx queue 7 */
#define ETH_TQCR_DSQ7		(1<<15)	/* Disable Tx queue 7 */

static void ethernet_phy_setup_link(int eth_port_num)
{
	unsigned int phy_reg_data;
	unsigned int reg_data;
	unsigned int port_enabled;
	unsigned long wait;

	/* Mask link/speed/duplex change interrupts at the PHY */
	ethernet_phy_interrupt_disable(eth_port_num);

	/* Read the Port Serial Control Register */
	reg_data =
	    mv_read(MV643XX_ETH_PORT_SERIAL_CONTROL_REG(eth_port_num));
	/* Extract the "port enabled" bit */
	port_enabled = reg_data & ETH_SERIAL_PORT_ENABLE;
	reg_data &= ~ETH_SERIAL_PORT_ENABLE;

	/* shutdown transmission if port is enabled */
	if (port_enabled) {
		/* Disable transmission by writing the "disable queue" bit */
		mv_write(MV643XX_ETH_TRANSMIT_QUEUE_COMMAND_REG(eth_port_num),
		  ETH_TQCR_DSQ7);

		/* Poll the "enable queue" bit to verify */
		for (wait = 2000000; wait > 0; wait -= 1000) {
			if (mv_read(MV643XX_ETH_TRANSMIT_QUEUE_COMMAND_REG
			     (eth_port_num)) & ETH_TQCR_ENQ7) {
				udelay(1000);
			} else {
				break;
			}
		...
		if (wait <= 0) {
			printk(KERN_ERR "mv643xx_eth: Unable to disable Tx\n");
			return;
		}

		/* Poll the Port Status Reg to ensure the Tx FIFO is empty */
		for (wait = 1000000; wait > 0; wait -= 1000) {
			if (mv_read(MV643XX_ETH_PORT_STATUS_REG(eth_port_num))
		...
		if (wait <= 0) {
			printk(KERN_ERR
			       "mv643xx_eth: Unable to terminate Tx\n");
			return;
		}
	}

	/* Set bits to disable auto-negotiation */
	reg_data |=
	    (ETH_DISABLE_AUTO_NEG_SPEED_GMII |
	     ETH_DISABLE_AUTO_NEG_FOR_FLOW_CTRL |
	     ETH_DISABLE_AUTO_NEG_FOR_DUPLX);

	/* Clear speed and duplex bits for now */
	reg_data &=
	    ~(ETH_SET_MII_SPEED_TO_100 |
	      ETH_SET_GMII_SPEED_TO_1000 | ETH_SET_FULL_DUPLEX_MODE);

	/* read 1000T status register to find link partner's 1000Mb abilities */
	eth_port_read_smi_reg(eth_port_num, DSC2_ENET_PHY_1000T_STATUS,
			      &phy_reg_data);
	/* Set to 1000Mbps full duplex if link partner is able */
	if (phy_reg_data & DSC2_ENET_PHY_ST1000T_LPA_1000T_FD) {
		reg_data |= ETH_SET_GMII_SPEED_TO_1000;
		reg_data |= ETH_SET_FULL_DUPLEX_MODE;
	} else {
		/* read the PHY's Auto-Negotiation Link Partner Ability register 
		   * to determine link partner's 10/100 abilities */
		eth_port_read_smi_reg(eth_port_num, DSC2_ENET_PHY_AUTONEG_LPA,
				      &phy_reg_data);
		if (phy_reg_data & DSC2_ENET_PHY_AUTONEG_LPA_100TX_FD) {
			/* 100Mb full duplex */
			reg_data |= ETH_SET_MII_SPEED_TO_100;
			reg_data |= ETH_SET_FULL_DUPLEX_MODE;
		} else if (phy_reg_data & DSC2_ENET_PHY_AUTONEG_LPA_100TX_HD) {
			/* 100Mb half duplex */
			reg_data |= ETH_SET_MII_SPEED_TO_100;
		} else if (phy_reg_data & DSC2_ENET_PHY_AUTONEG_LPA_10T_FD) {
			/* 10Mb full duplex */
			reg_data |= ETH_SET_FULL_DUPLEX_MODE;
		} else {
	...
	/* Read PHY status register for link up/down */
	eth_port_read_smi_reg(eth_port_num, DSC2_ENET_PHY_MII_STATUS,
			      &phy_reg_data);
	if (phy_reg_data & DSC2_ENET_PHY_ST_LINK_UP) {
		/* force link up */
		reg_data |= ETH_FORCE_LINK_PASS;
		reg_data |= ETH_DO_NOT_FORCE_LINK_FAIL;
	} else {
	...
	/* Enable link/speed/duplex change interrupts at the PHY */
	ethernet_phy_interrupt_enable(eth_port_num);

	/* Reenable port if previously enabled */
	if (port_enabled) {
		mv_write(MV643XX_ETH_PORT_SERIAL_CONTROL_REG(eth_port_num),
			     reg_data | port_enabled);
	}
	return;
}
#endif /* CONFIG_POWERPMC280 */
