...
static struct constraints pxacmc91x_constraints = {
        .count =  1,
        .param = {{DPM_MD_SLEEP_MODE, 0, 0},},
...
#if defined(CONFIG_SMP) || defined(CONFIG_NET_POLL_CONTROLLER)
/*
...
static int smc_hard_do_xmit(struct sk_buff *skb,
			    ...
			    int wait_forever)
{
	...
	do {
		...
		if (unlikely(wait_forever))
		{
			/* Poll forever! */
			poll_count = MEMORY_WAIT_TIME;
			if (status & IM_RCV_INT) {
				/* Receive some packets to get more memory freed up */
				smc_rcv(dev);
			}
		}
	} while (--poll_count);
...
static int smc_hard_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	if (lp->pending_tx_skb != NULL) {
        ...
        /* Make an attempt to send this buffer first and then swap
         * back to the old one as we are probably in polling mode.
         */
        struct sk_buff *old_skb;

        old_skb = lp->pending_tx_skb;
        smc_hard_do_xmit(skb, dev, lp, 1);
        lp->pending_tx_skb = old_skb;
        return 0;
#else /* ! CONFIG_NET_POLL_CONTROLLER */
        printk(KERN_ERR "smc91x: BUG! No TX Resource, refusing to send buffer.\n");
        netif_stop_queue(dev);
        return NETDEV_TX_BUSY;
#endif /* ! CONFIG_NET_POLL_CONTROLLER */
	}
#endif /* 0 */
    return smc_hard_do_xmit(skb, dev, lp, 0);
}
...
#if defined(CONFIG_DPM) && defined(CONFIG_MACH_MAINSTONE)
	pdev->dev.constraints=&pxacmc91x_constraints;
