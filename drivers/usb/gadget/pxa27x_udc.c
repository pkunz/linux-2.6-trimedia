...
#define	DMASTR " (pio only) some gadgets may not work"
#endif
...
static struct constraints pxa_udc_constraints = {
        /* one constraint */
        .count =  1,
...
#if defined(CONFIG_DPM) && defined(CONFIG_MACH_MAINSTONE)
	pdev->dev.constraints=&pxa_udc_constraints;
...
static int pxa27x_udc_suspend(struct device *_dev, u32 state, u32 level)
{
	...
	if (dev->suspended == 0 && level == SUSPEND_DISABLE) {
		local_irq_save(flags);
		UDCCR &= ~UDCCR_UDE;
		pxa_set_cken(CKEN11_USB, 0);
		make_usb_disappear();
...
static int pxa27x_udc_resume(struct device *_dev, u32 level)
{
	...
	if (dev->suspended == 1 && level == RESUME_ENABLE) {
		local_irq_save(flags);
		make_usb_disappear();
		udc_ep_setup();

		/* the OTGPH bit should be dropped upon exit from sleep mode	
		 * see PXA27x Developer's Manual 3.8.1.2
		 */
		PSSR  |= PSSR_OTGPH;
