...
#define CACULATE_SYSCALL_ROUTINE  						\
	(* cycle 0 *)  								\
	IF r1   uimm(SYMBOL_NAME(sys_ni_syscall)) -> REG_temp1,          	\
	IF r1   uimm(SYMBOL_NAME(sys_call_table)) -> REG_temp2,        		\
	IF r1   uimm(NR_syscalls) -> REG_temp3,                  		\
	IF r1   nop,  								\
	IF r1   nop;  								\
  										\
	(* cycle 1 *)  								\
	...
	IF r1   nop,  								\
	IF r1   nop,  								\
	IF r1   ld32x REG_temp2 REG_SYSCALL_NR -> REG_temp4,                  	\
	IF r1   nop;  								\
  										\
	(* cycle 2 *)  								\
	IF r1   nop,  								\
	...
	IF r1   nop;  								\
  										\
	(* cycle 3 *)  								\
	IF r1   ugtr REG_temp3 REG_SYSCALL_NR -> REG_temp2,                  	\
	IF r1   nop,  								\
	IF r1   nop,  								\
	IF r1   nop,  								\
	IF r1   nop;  								\
  										\
	(* cycle 4 *)  								\
	IF REG_temp2   iadd r0 REG_temp4 -> REG_temp1,                		\
	IF r1   nop,  								\
	IF r1   nop,  								\
	IF r1   nop,  								\
	IF r1   nop;  								\
  										\
	(* cycle 5 *)  								\
	IF r1   iadd r0 REG_temp1 -> REG_SYSCALL_ROUTINE,              		\
	IF r1   nop,  								\
	IF r1   nop,  								\
	IF r1   nop,  								\
	IF r1   nop; 	 
...
#define SET_SYSCALL_RP   							\
        IF r1   uimm(SYMBOL_NAME(ret_from_syscall)) -> r2,  			\
        IF r1   nop,  								\
        IF r1   nop,  								\
        IF r1   nop,  								\
        IF r1   nop;
...
ENTRY(ret_from_syscall)
	...
	*/
	...
	/*
	* So that after end_of_interrup, common_intr_exit will be called
	*/
	(* cycle 2 *)
        IF r1   uimm(SYMBOL_NAME(common_intr_exit)) -> r2, 
        IF r1   nop,
        IF r1   nop,     
        IF r1   nop,    
	IF r1   nop; 
