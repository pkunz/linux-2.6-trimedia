...
extern int via686found;		/* Declared in pci_auto.c */
void __init
mpc85xx_cds_init_IRQ(void)
{
	...
	if(via686found) {
		printk(KERN_INFO "VIA82C686 chip found, install its ISA devices interrupts.\n");
		openpic_hookup_cascade(PIRQ0A, "8259 cascade to OpenPCI",
				&i8259_irq);

		for (i = i8259_OFFSET; i < i8259_OFFSET + NUM_8259_INTERRUPTS;
				i++)
			irq_desc[i].handler = &i8259_pic;

		i8259_pic_irq_offset = i8259_OFFSET;
...
mpc85xx_map_irq(struct pci_dev *dev, unsigned char idsel, unsigned char pin)
{
	...
	if (!hose->index)
	{
		/* Handle PCI1 interrupts */
			 ...
			 * interrupt on slot */
#if defined(CONFIG_ARCADIA_X3)
		char (*pci_irq_table)[4];

		char pci_irq_table_parallel[][4] =
		{
			{ 0, 1, 2, 3 },	/* 2 - pci-x Tsi310 Bridge */
			{ 0, 1, 2, 3 },	/* 3 - pci-x Slot 2 */
			{ 0, 1, 2, 3 },	/* 4 - pci-x Slot 3 */
			{ 0, 1, 2, 3 },	/* 5 - pci-x Slot 4 */
			{ 0, 1, 2, 3 },	/* 6 - pci-x Slot 5 */
			{ 0, 0, 0, 0 },	/* 7 -- */
			{ 0, 0, 0, 0 },	/* 8 -- */
			{ 0, 0, 0, 0 },	/* 9 -- */
			{ 0, 0, 0, 0 },	/* 10-- */
		...
		char pci_irq_table_rotated[][4] =
		{
			{ 0, 0, 0, 0 },	/* 2 -- */
			{ 0, 0, 0, 0 },	/* 3 -- */
			{ 0, 1, 2, 3 },	/* 4 - pci-x Slot 1 */
			{ 1, 2, 3, 0 },	/* 5 - pci-x Slot 2 */
			{ 2, 3, 0, 1 },	/* 6 - pci-x Slot 3 */
			{ 0, 0, 0, 0 },	/* 7 -- */
			{ 0, 1, 2, 3 },	/* 8 - pci-x Slot 4 */
			{ 0, 0, 0, 0 },	/* 9 -- */
			{ 0, 0, 0, 0 },	/* 10-- */
			{ 0, 0, 0, 0 },	/* 11-- */
			{ 0, 0, 0, 0 },	/* 12-- */
		...
		const long min_idsel = 2, max_idsel = 28, irqs_per_slot = 4;
		int i, j;

		dev = pci_find_device(0x1957, 0x3fff, NULL);
		if (dev != NULL) {
			pci_read_config_dword(dev, PCI_CLASS_REVISION, &class_rev);
			if ((class_rev & 0xff)< 0x31) {
				pci_irq_table = pci_irq_table_parallel; /* Arcadia X3.0 */
			}
			else {
				pci_irq_table = pci_irq_table_rotated; /* Arcadia X3.1 */
			}
		}
		else {
			pci_irq_table = pci_irq_table_parallel; /* not found ARCADIA on PCI bus, presume X3.0 */
		}

		for (i = 0; i < 27; i++ )
			for (j = 0; j < 4; j++)
				pci_irq_table[i][j] = pci_irq_table[i][j] + PIRQ0A;
...
#ifdef CONFIG_ARCADIA_X3
#define ARCADIA_HOST_BRIDGE_IDSEL	18
#define ARCADIA_2ND_BRIDGE_IDSEL	17
#else
...
mpc85xx_cds_setup_arch(void)
{
	...
	printk("Bus frequency is %d\n", freq);

	/* get the core frequency */
	freq = binfo->bi_intfreq;
	printk("Core frequency is %d\n",freq);
