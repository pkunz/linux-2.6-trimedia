...
fcc_enet_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	/* We have to wait forever until the buffer is ready, when we
	 * are the polling driver.
	 */
	while (bdp->cbd_sc & BD_ENET_TX_READY);
#endif
...
static void fcc_enet_poll (struct net_device *dev)
{
	struct fcc_enet_private * cep = (struct fcc_enet_private *)dev->priv;
        volatile cbd_t  *bdp = cep->cur_tx;

        /* disable_irq is not very nice, but with the funny lockless design
           we have no other choice. */
        disable_irq(dev->irq);

        /* We must always wait for the TX buffer to be clear */
        while (bdp->cbd_sc & BD_ENET_TX_READY);

        fcc_enet_interrupt (dev->irq, dev, NULL);
        enable_irq(dev->irq);
}
...
#ifdef CONFIG_NET_POLL_CONTROLLER
		dev->poll_controller = fcc_enet_poll;
#endif  /* CONFIG_NET_POLL_CONTROLLER */
