			       ...
			       struct pt_regs *linux_regs)
{
	...
	gdb_cont_from_kill = 0;
	switch (remcom_in_buffer[0]) {
	case 'k':
		gdb_cont_from_kill = 1;
	case 'c':
	case 's':
		       ...
		       void *ptr)
{
	...
	/* If KGDB I/O module is not connected return immediately */
	if (!kgdb_connected)
		return NOTIFY_DONE;
	...
	if (gdb_cont_from_kill)
	{
		gdb_cont_from_kill = 0;
		/* We hit a breakpoint so we should not NOTIFY_DONE */
		if (!(args->signr == SIGTRAP && args->trapnr == 3))
			return NOTIFY_DONE;
	}
	return NOTIFY_STOP;
}
