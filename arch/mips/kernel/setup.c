...
#ifdef CONFIG_MIPS_BRCM
	/* THT PR10078: We initialize the entire RAM, but will reserve the upper memory region */
	for (i = 0; i < boot_mem_map.nr_map; i++) {
		if (boot_mem_map.map[i].type != BOOT_MEM_RAM)
			continue;
		/* 
		 * We have registed only 2 maps, one starting at 0, the other at the end 
		 * of first area (BOOT_MEM_RAM type)
		 */
		if (boot_mem_map.map[i].addr != 0) {
			reserve_bootmem(boot_mem_map.map[i].addr, 
				boot_mem_map.map[i].size);
			if (upper_memory < boot_mem_map.map[i].addr) {
				upper_memory = boot_mem_map.map[i].addr;
			}
			printk(KERN_NOTICE "Reserving %ld MB upper memory starting at %08x\n", 
				boot_mem_map.map[i].size >> 20,
