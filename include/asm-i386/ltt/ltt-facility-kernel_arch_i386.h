...
enum lttng_syscall_name {
	...
	LTTNG_tee = 315,
	LTTNG_vmsplice = 316,
	LTTNG_ltt_trace_generic = 317,
	LTTNG_ltt_register_generic = 318,
};
...
#else
{
	...
	*len += sizeof(const void *);
	...
	index = ltt_get_index_from_facility(ltt_facility_kernel_arch_BDE45AD9,
	...
	list_for_each_entry_rcu(trace, &ltt_traces.head, list) {
		...
		*to_base = *to = *len = 0;
		...
		ltt_write_event_header(trace, channel, buffer,
			ltt_facility_kernel_arch_BDE45AD9, event_kernel_arch_syscall_entry,
...
#else
{
	...
	list_for_each_entry_rcu(trace, &ltt_traces.head, list) {
		...
		*to_base = *to = *len = 0;
		...
		ltt_write_event_header(trace, channel, buffer,
			ltt_facility_kernel_arch_BDE45AD9, event_kernel_arch_syscall_exit,
			reserve_size, before_hdr_pad, tsc);
		*to_base += before_hdr_pad + after_hdr_pad + header_size;
