...
#define alloc_thread_info(tsk)						\
	({								\
		struct thread_info *ret;				\
									\
		ret = __get_free_pages(GFP_KERNEL, THREAD_SIZE_ORDER);	\
		if (ret)						\
			memset(ret, 0, THREAD_SIZE);			\
		ret;							\
	})
#else
#define alloc_thread_info(tsk) __get_free_pages(GFP_KERNEL, THREAD_SIZE_ORDER)
#endif

#define free_thread_info(info) free_pages(info, THREAD_SIZE_ORDER);
