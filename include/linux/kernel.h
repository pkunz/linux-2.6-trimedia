...
asmlinkage int t_printk(const char * fmt, ...)
	__attribute__ ((format (printf, 1, 2)));
#else
static inline int vprintk(const char *s, va_list args)
	__attribute__ ((format (printf, 1, 0)));
static inline int vprintk(const char *s, va_list args) { return 0; }
static inline int t_printk(const char *s, ...)
	__attribute__ ((format (printf, 1, 2)));
static inline int t_printk(const char *s, ...) { return 0; }
#endif
...
#define printk(fmt, args...)  				\
	({ 						\
	 	int __print_len; 			\
		__print_len = printf(fmt, ## args); 	\
		__print_len = t_printk(fmt, ## args); 	\
		__print_len; 				\
	})
#else
