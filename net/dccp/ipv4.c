					   ...
					   struct sk_buff *skb)
{
	...
	fl.oif=((struct rtable *)skb->dst)->rt_iif;
	fl.nl_u.ip4_u.daddr=skb->nh.iph->saddr;
	fl.nl_u.ip4_u.saddr=skb->nh.iph->daddr;
	fl.nl_u.ip4_u.tos=RT_CONN_FLAGS(sk);
	fl.proto=sk->sk_protocol;
	fl.uli_u.ports.sport=dccp_hdr(skb)->dccph_dport;
	fl.uli_u.ports.dport=dccp_hdr(skb)->dccph_sport;
#endif 
