		       ...
		       struct in6_addr *saddr)
{
	...
	for (dev = dev_base; dev; dev=dev->next) {
		...
		for (ifp=ifp0; ifp; ifp=ifp->if_next) {
			...
			if (ipv6_addr_equal(&ifp->addr, daddr))
			...
			if (ifp->flags & IFA_F_HOMEADDRESS)
	...
	if (candidate.ifp) {
		 ...
		 */
		if (sprefix &&
		    !ipv6_prefix_equal(&candidate.ifp->addr,
...
static void addrconf_add_mroute(struct net_device *dev)
{
	...
	ip6_route_add(&rtmsg, NULL, NULL, NULL,
		      rt6_dev_dflt_mcast_table_id(dev->ifindex));
}
	...
	if (pinfo->onlink) {
		...
		if (rt && ((rt->rt6i_flags & (RTF_GATEWAY | RTF_DEFAULT)) == 0)) {
			...
			addrconf_prefix_route(&pinfo->prefix, pinfo->prefix_len,
					      dev, jiffies_to_clock_t(rt_expires), 
			  ...
			  unsigned flags, __u32 prefered_lft, __u32 valid_lft)
{
	...
	ifp = ipv6_add_addr(idev, pfx, plen, scope, flags|permanent);
	if (!IS_ERR(ifp)) {
