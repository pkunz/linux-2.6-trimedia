...
int ip_queue_xmit(struct sk_buff *skb, int ipfragok)
{
	...
	if (rt == NULL) {
...

		{
			...
			fl.nl_u.ip4_u.saddr=inet->saddr;
...
#else
				inet->cork.opt = kmalloc(sizeof(struct ip_options) - sizeof(unsigned char) + 40, sk->sk_allocation);
#endif 
...
#else
			memcpy(inet->cork.opt, opt, sizeof(struct ip_options)-sizeof(unsigned char)+opt->optlen);
#endif 
		   ...
		   unsigned int len)
{
...

	{
		...
		memset(&fl,0,sizeof(fl));
		fl.nl_u.ip4_u.daddr=daddr;
		fl.nl_u.ip4_u.saddr=rt->rt_spec_dst;
		fl.nl_u.ip4_u.tos=RT_TOS(skb->nh.iph->tos);
		fl.uli_u.ports.sport=skb->h.th->dest;
		fl.uli_u.ports.dport=skb->h.th->source;
		fl.proto=sk->sk_protocol;
#endif 
