...
static struct sk_buff *igmpv3_newpack(struct net_device *dev, int size)
{
...

	{
		...
		fl.oif=dev->ifindex;
		fl.nl_u.ip4_u.daddr=IGMPV3_ALL_MCR;
...
#else
	return sizeof(struct igmpv3_grec) - sizeof(__u32) + 4*igmp_scount(pmc,type,gdel,sdel);
#endif 
}
...
#else
	pgr = (struct igmpv3_grec *)skb_put(skb, sizeof(struct igmpv3_grec) - sizeof(__u32));
#endif 
	pgr->grec_type = type;
	pgr->grec_auxwords = 0;
	pgr->grec_nsrcs = 0;
...
#else
			if (skb && AVAILABLE(skb) < sizeof(struct igmpv3_grec) - sizeof(__u32) +
			    sizeof(__u32)) {
				igmpv3_sendpack(skb);
				skb = NULL; /* add_grhead will get a new one */
			}
...
#else
		if (AVAILABLE(skb) < sizeof(u32) +
		    first*(sizeof(struct igmpv3_grec)-sizeof(__u32))) {
			if (truncate && !first)
				break;	 /* truncate these */
			if (pgr)
				pgr->grec_nsrcs = htons(scount);
