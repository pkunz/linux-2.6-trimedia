			...
			struct net_device *dev, u32 *spec_dst, u32 *itag)
{
	...
	struct flowi fl;
	memset(&fl,0,sizeof(fl));
	fl.nl_u.ip4_u.daddr=src;
	fl.nl_u.ip4_u.saddr=dst;
...
static void nl_fib_lookup(struct fib_result_nl *frn, struct fib_table *tb )
{
	...
	struct flowi 		fl;
	memset(&fl,0,sizeof(fl));
	fl.nl_u.ip4_u.daddr=frn->fl_addr;
	fl.nl_u.ip4_u.fwmark=frn->fl_fwmark;
	fl.nl_u.ip4_u.tos=frn->fl_tos;
	fl.nl_u.ip4_u.scope=frn->fl_scope;
#endif 
	if (tb) {
		local_bh_disable();
