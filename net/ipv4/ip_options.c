...
#else
	memcpy(&(IPCB(skb)->opt), opt, sizeof(struct ip_options)-sizeof(unsigned char));
#endif 
...
#else
	memset(dopt, 0, sizeof(struct ip_options)-sizeof(unsigned char));
#endif 
...
#else
		memset(opt, 0, sizeof(struct ip_options)-sizeof(unsigned char));
#endif 
		iph = skb->nh.raw;
...
static struct ip_options *ip_options_get_alloc(const int optlen)
{
	...
	struct ip_options *opt = kmalloc(sizeof(*opt) + ((optlen + 3) & ~3),
					 GFP_KERNEL);
#else
	struct ip_options *opt = kmalloc(sizeof(*opt) - sizeof(unsigned char) + ((optlen + 3) & ~3),
					 GFP_KERNEL);
#endif 
	if (opt)
