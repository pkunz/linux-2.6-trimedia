...
static int km_query(struct xfrm_state *x, struct xfrm_tmpl *t, struct xfrm_policy *pol, int dir);
...
#ifdef CONFIG_XFRM_ENHANCEMENT
int km_report(__u8 dir, __u8 proto, struct xfrm_selector *sel, void *arg)
{
	int err = -EINVAL;
	int ret;
	struct xfrm_mgr *km;
	...
	list_for_each_entry(km, &xfrm_km_list, list) {
		if (km->report) {
			ret = km->report(dir, proto, sel, arg);
			if (!ret)
				err = ret;
		}
	}
