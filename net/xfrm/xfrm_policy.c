...
xfrm_post_reject(struct xfrm_state *x, struct sk_buff *skb, struct flowi *fl)
{
	if (!x || !x->type->post_reject)
		return 0;
	return x->type->post_reject(x, NULL, skb, fl);
}
#endif
...
#endif
{
	...
	if (tmpl->optional) {
#ifdef CONFIG_XFRM_ENHANCEMENT
		if (tmpl->mode == XFRM_MODE_ROUTEOPTIMIZATION_TRIGGER) {
	...
	for (; idx < sp->len; idx++) {
		if (xfrm_state_ok(tmpl, sp->x[idx].xvec, family))
			return ++idx;
#ifdef CONFIG_XFRM_ENHANCEMENT
		if (sp->x[idx].xvec->props.mode != XFRM_MODE_TRANSPORT) {
			*xerrp = sp->x[idx].xvec;
			break;
		}
#else
...
#endif
{
	for (; k < sp->len; k++) {
#ifdef CONFIG_XFRM_ENHANCEMENT
		if (sp->x[k].xvec->props.mode != XFRM_MODE_TRANSPORT) {
			*xerrp = sp->x[k].xvec;
			return 1;
		}
#else
