...
static int dn_route_output_slow(struct dst_entry **pprt, const struct flowi *oldflp, int try_hard)
{
	...
	fl.nl_u.dn_u.daddr=oldflp->fld_dst;
	fl.nl_u.dn_u.saddr=oldflp->fld_src;
	fl.nl_u.dn_u.scope=RT_SCOPE_UNIVERSE;
#ifdef CONFIG_DECNET_ROUTE_FWMARK
	fl.nl_u.dn_u.fwmark=oldflp->fld_fwmark;
#endif 
	fl.iff=loopback_dev.ifindex;
	fl.oif=oldflp->oif;
#endif 
...
static int dn_route_input_slow(struct sk_buff *skb)
{
	...
	struct flowi fl;
	memset(&fl,0,sizeof(fl));
	fl.nl_u.dn_u.daddr=cb->dst;
	fl.nl_u.dn_u.saddr=cb->src;
	fl.nl_u.dn_u.scope=RT_SCOPE_UNIVERSE;
#ifdef CONFIG_DECNET_ROUTE_FWMARK
	fl.nl_u.dn_u.fwmark=skb->nfmark;
#endif 
