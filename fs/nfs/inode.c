...
static struct rpc_program	alt_nfs_program;
...
static struct rpc_stat		alt_nfs_rpcstat = {
	.program		= &alt_nfs_program
};
...
nfs_create_client(struct nfs_server *server, const struct nfs_mount_data *data)
{
	...
	if (data->version == 7) {
		alt_nfs_program.number = data->nfs_prog;
		clnt = rpc_create_client(xprt, server->hostname, &alt_nfs_program,
...
nfs_statfs(struct super_block *sb, struct kstatfs *buf)
{
	 ...
	 */
	buf->f_frsize = nfs_block_bits(server->wtmult,&blockbits);
