...
#ifndef CONFIG_DYNLOAD
extern struct linux_binfmt fn_format;

/*
 * cycle the list of binary formats handler, until one recognizes the image
 */
int search_fn_binary_handler(struct linux_binprm *bprm,struct pt_regs *regs)
{
	int try,retval;
	struct linux_binfmt *fmt;
...
/*
 * sys_fn_execve() executes a new userspace process from a function pointer.
 */
int
do_fn_execve(unsigned long fn,
       ...
       struct pt_regs* regs)
{
       ...
       retval = search_fn_binary_handler(bprm,regs);
       if (retval >= 0) {
               free_arg_pages(bprm);
