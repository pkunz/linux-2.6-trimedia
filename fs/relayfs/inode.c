...
{
	printk(KERN_WARNING "Relayfs ioctl unimplemented\n");
	return -EPERM;
}
...
void setup_fileops(struct rchan *chan,
				   struct file_operations *fops)
{
	memset(&chan->fops, 0, sizeof(chan->fops));
	
	chan->fops.open = fops->open;
	chan->fops.poll = fops->poll;
	chan->fops.mmap = fops->mmap;
	chan->fops.release = fops->release;
	chan->fops.ioctl = fops->ioctl;
	chan->fops.llseek = fops->llseek;
	chan->fops.read = fops->read;
	
	if (!chan->fops.open)
		chan->fops.open = relayfs_open;
	if (!chan->fops.poll)
		chan->fops.poll = relayfs_poll;
	if (!chan->fops.mmap)
		chan->fops.mmap = relayfs_mmap;
	if (!chan->fops.read)
		chan->fops.read = relayfs_read;
	if (!chan->fops.llseek)
		chan->fops.llseek = no_llseek;
	if (!chan->fops.release)
		chan->fops.release = relayfs_release;
	if (!chan->fops.ioctl)
		chan->fops.ioctl = relayfs_ioctl;
}

/* structure imported from 2.6.16; .ioctl not set! */
